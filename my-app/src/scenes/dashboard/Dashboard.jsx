  ...
  function Dashboard() {
    ...
    const handleCloseWelcomeModal = () => {
      ...
      const barChartDataMonth = gastosFiltrados.reduce((acc, gasto) => {
        const month = new Date(gasto.data).toLocaleString('default', { month: 'long' });
        ...
        if (gasto.valor >= 0) {
            acc[month].entradas += gasto.valor;
        } else {
            acc[month].saidas += Math.abs(gasto.valor); // Abs para exibir saídas como positivo
        }
